...
def queryBlat(chatstatus):
    """
    Queries the BLAT database for DNA sequences provided in the `chatstatus` and updates the status with results.

    This function extracts a prompt from the `chatstatus` dictionary, cleans the prompt by removing 
    unnecessary punctuation, and then performs BLAT searches on DNA sequences found in the cleaned prompt. 
    ...
    Returns:
    dict: The updated `chatstatus` dictionary, which includes the results of the BLAT queries appended to the 'process' key.
...
def dna_check(s):
    """
    Check if the given string contains only the characters 'A', 'C', 'T', and 'G'.

    This function takes a string as input and determines whether every character
    in the string is one of the four allowed characters: 'A', 'C', 'T', or 'G'.
    It returns `True` if all characters in the string are among the allowed characters,
    and `False` otherwise.
    ...
    Returns:
    bool: `True` if all characters in the string are 'A', 'C', 'T', or 'G';
          `False` otherwise.
    """
    allowed_characters = {'A', 'C', 'T', 'G'}
    return set(s).issubset(allowed_characters)
