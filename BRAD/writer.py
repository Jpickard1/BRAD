...
def setTitle(chatstatus, chatlog):
    ...
    for promptNumber in chatlog.keys():
        log.debugLog(promptNumber, chatstatus=chatstatus) # <- This is for debugging
        if promptNumber == 'llm':
            continue
...
def getProcessSummary(chatstatus, chatlog):
    ...
    log.debugLog(fullPlanner, chatstatus=chatstatus) # <- This is for debugging
...
def databaseReporter(chatlogStep, chatstatus):
    ...
    for step in chatlogStep['process']['steps']:
        ...
        if 'func' in step.keys() and step['func'] == 'utils.save':
            ...
            chatstatus = log.userOutput(filename, chatstatus=chatstatus) # <- This is for the user
...
def addTables(chatlogStep):
    ...
    if df is None:
        return ""
    log.debugLog(type(df), chatstatus=chatstatus) # <- This is for debugging
    log.debugLog(df.head(3), chatstatus=chatstatus) # <- This is for debugging
