...
def runPrompt(prompt,
              ...
              numArticles      = 5,                        # number of articles to retrieve
              numRagCalls      = 12,                       # Number of trials with the RAG
    ...
    for i in range(len(docs)):
        outputs['doc-'+str(i)] = docs[i].page_content
    for i in range(numRagCalls):
        # Call LLM
...
def main():
    ...
    for i, prompt in enumerate(prompts):
        ...
        runPrompt(prompt,
                  ...
                  vectordb,
                  numArticles      = 5,                       # number of articles to retrieve
                  numRagCalls      = 5,                       # Number of trials without the RAG
                  llmOnlyOutputFile='EX-12-JUNE-2024-RAG-TEMP-llama2-7b-temp-' + str(temp) + '.csv') # Outputs of the LLM without the RAG
